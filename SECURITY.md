# Security Policy

We take the security of MadeOnShroom and its users seriously. Because the platform is non-custodial,
all value-bearing logic settles on public Solana programs — but we still want to hear about any issue
that could put users at risk.

## Reporting a vulnerability

If you discover a security vulnerability, please report it **privately**. Do **not** open a public
GitHub issue for security problems.

- **Email:** madeonshroom@outlook.com
- Please include a clear description, reproduction steps, and the potential impact.
- If possible, include the affected URL, transaction signature, or token/pool address.

We aim to acknowledge reports promptly and will keep you informed as we investigate.

## Responsible disclosure

- Give us a reasonable window to investigate and remediate before any public disclosure.
- Do not access, modify, or destroy data that is not yours.
- Do not run attacks that could degrade service for other users (e.g. spam, denial of service).
- Act in good faith and avoid privacy violations.

## Scope

In scope:

- The MadeOnShroom website (https://madeonshroom.com) and its API surface.
- The native mobile apps (iOS & Android).

Out of scope:

- The Meteora DBC and DAMM v2 programs themselves (report those to Meteora).
- The Solana network, validators, and RPC providers.
- Issues in third-party wallets (e.g. Phantom).
- Volatility, market, or financial loss inherent to trading speculative tokens.

## A note on custody

MadeOnShroom never holds your SOL, tokens, or private keys. Always verify transactions in your wallet
before signing, and never share your seed phrase or private key with anyone — including anyone
claiming to be from MadeOnShroom.
